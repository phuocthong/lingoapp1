...
const taskRoutes = new Elysia({ prefix: '/tasks' })
  .get('/', async ({ user }) => {
    try {
      ...
      const tasksWithProgress = await db
        ...
        .from(tasks)
        .leftJoin(userTaskProgress, and(
          eq(userTaskProgress.taskId, tasks.id),
          eq(userTaskProgress.userId, user.id)
        ))
  ...
  .post('/:id/progress', async ({ params, body, user, set }) => {
    try {
      const taskId = parseInt(params.id)
      const { increment = 1 } = body

      // Get or create user task progress
      let progress = await db.select().from(userTaskProgress)
        .where(and(
          eq(userTaskProgress.taskId, taskId),
          eq(userTaskProgress.userId, user.id)
        ))
      ...
      if (!progress) {
        ...
        progress = await db.insert(userTaskProgress).values({
          userId: user.id,
          taskId,
          current: 0,
          completed: false,
