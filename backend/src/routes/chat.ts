...
export const chatRoutes = new Elysia({ prefix: '/api/chat' })
  ...
  // Nộp kết quả battle
  .post('/battle/:id/submit', ({ params, body, user, set }) => {
    try {
      ...
      for (const answer of answers) {
        ...
        const isCorrect = answer.user_answer.toLowerCase().trim() === question.correct_answer.toLowerCase().trim();
      ...
      if (battle.mode === 'bot') {
        ...
        const botScore = Math.floor(Math.random() * (totalScore * 1.2)) + Math.floor(totalScore * 0.3);
        ...
        if (isPlayer1) {
          db.prepare('UPDATE battles SET player1_answers = ? WHERE id = ?')
            .run(JSON.stringify(detailedResults), battleId);
        } else {
          db.prepare('UPDATE battles SET player2_answers = ? WHERE id = ?')
            .run(JSON.stringify(detailedResults), battleId);
        }
