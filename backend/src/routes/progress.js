...
const progressRoutes = new Elysia({ prefix: '/progress' })
  .get('/', async ({ user, query }) => {
    try {
      ...
      const totalStats = progressData.reduce((acc, day) => ({
      ...
      const accuracy = totalStats.questionsAnswered > 0 
        ? (totalStats.correctAnswers / totalStats.questionsAnswered) * 100 
        : 0

      const averageTime = totalStats.questionsAnswered > 0
        ? totalStats.timeSpent / totalStats.questionsAnswered
        : 0
  ...
  .post('/record', async ({ body, user, set }) => {
    try {
      const { 
        questionsAnswered, 
        correctAnswers, 
