...
export const leaderboardRoutes = new Elysia({ prefix: '/api/leaderboard' })
    ...
    try {
      ...
      if (user) {
        ...
        userRanking = {
          ...user,
          rank: ranking.rank,
          accuracy_rate: user.total_questions_answered > 0 
  ...
  .get('/category/:category', ({ params, query }) => {
    try {
      const { category } = params;
      const { limit = 50 } = query;
      const limitNum = Math.min(Number(limit), 100);
      
      const categoryLeaderboard = db.prepare(`
        SELECT 
  ...
  .get('/friends', ({ user, query }) => {
    try {
      ...
      const friendsLeaderboard = db.prepare(`
        SELECT 
