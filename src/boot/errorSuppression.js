...
(function() {
  ...
  console.error = function(...args) {
    ...
    if (typeof errorMessage === 'string') {
      ...
      if (errorMessage.includes('Cannot read properties of undefined') && errorMessage.includes('send')) {
        return
      }
  ...
  window.addEventListener('error', (event) => {
    if (event.message) {
      ...
      if (event.message.includes('Cannot read properties of undefined') && event.message.includes('send')) {
        event.preventDefault()
        event.stopPropagation()
        return false
      }
  ...
  window.addEventListener('unhandledrejection', (event) => {
    if (event.reason && event.reason.message) {
      ...
      if (event.reason.message.includes('WebSocket') ||
          ...
          event.reason.message.includes('Cannot read properties of undefined') && event.reason.message.includes('send')) {
        event.preventDefault()
        return false
      }
    }
